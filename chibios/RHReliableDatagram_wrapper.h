...
void RHReliableDatagram_init(void);
...
uint8_t RHReliableDatagram_retries(void);
...
///ChipEnablePin(chipE) You should be sure to call this function frequently enough to not miss any messages
/// It is recommended that you call it in your main loop.
/// \param[in] buf Location to copy the received message
/// \param[in,out] len Available space in buf. Set to the actual number of octets copied.
...
/// (not just those addressed to this node).
/// \return true if a valid message was copied to buf
bool RHReliableDatagram_recvfromAckTimeout(uint8_t* buf, uint8_t* len,
		uint16_t timeout, uint8_t* from, uint8_t* to,		uint8_t* id, uint8_t* flags);
...
uint32_t RHReliableDatagram_retransmissions(void);

/// Resets the count of the number of retransmissions
/// to 0.
void RHReliableDatagram_resetRetransmissions(void);

bool RHReliableDatagram_available(void);
